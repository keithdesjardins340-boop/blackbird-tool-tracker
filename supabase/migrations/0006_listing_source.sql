-- Track HOW a listing was matched, so description-matched links (fuzzy) can be
-- flagged for the user to verify, distinct from SKU-matched and manual links.
alter table tool_listings add column if not exists source      text;          -- 'auto-sku' | 'auto-desc' | 'manual'
alter table tool_listings add column if not exists match_score numeric(4,3);  -- description confidence 0..1

-- Re-expose the market view with the best listing's source appended.
create or replace view tool_market_status with (security_invoker = true) as
with active_listings as (
  select
    tl.id as listing_id, tl.tool_id, tl.dealer_id, tl.product_url, tl.source,
